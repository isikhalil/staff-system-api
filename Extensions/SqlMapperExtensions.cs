...
namespace Dapper.Contrib.Extensions
{
	...
	public static partial class SqlMapperExtensions
	{
		...
		{
			var isVirtual = pi.GetGetMethod().IsVirtual;
		...
		private static string GetTableName(Type type)
		{
			...
			{
#if NETSTANDARD1_3 || NETCOREAPP2_1 || NET5_0
				...
				var tableSchemaName =
					info.GetCustomAttribute<TableAttribute>(false)?.Schema
					?? (info.GetCustomAttributes(false).FirstOrDefault(attr => attr.GetType().Name == "TableAttribute") as dynamic)?.Schema;

				if (tableAttrName != null)
				{
					name = string.Format("[{0}]", tableAttrName);

					if (!string.IsNullOrEmpty(tableSchemaName))
					{
						name = string.Format("[{0}].[{1}]", tableSchemaName, tableAttrName);
					}
				}
				else
				{
...
#if NETSTANDARD1_3 || NETSTANDARD2_0 || NETCORE2_1 || NETCOREAPP2_1 || NET5_0
...
#if NETSTANDARD1_3 || NETSTANDARD2_0 || NETCOREAPP2_1 || NET5_0
